...
namespace Mana // renamed from Supyrb
{
	using System;
	using UnityEngine;
	using UnityEditor;
	...
	/// <summary>
	/// Extension class for SerializedProperties that makes it easier to get and set values
	...
	public static class SerializedPropertyExtensions
	{
		...
		public static T GetNestedObject<T>(string path, object obj, bool includeAllBases = false)
		{
			...
			if (paths.Length > 0)
			{
				...
				if (paths[lastIndex].Contains("[") && paths[lastIndex].Contains("]"))
				{
					string arrayIndex = Regex.Replace(paths[lastIndex], @"[^\d]", "");
					if (!string.IsNullOrEmpty(arrayIndex))
					{
						string arrayName = paths[0];
						obj = GetFieldOrPropertyValue<object>(arrayName, obj, includeAllBases, Int32.Parse(arrayIndex));
						return (T)obj;
					}
				}
			}
		...
		public static T GetFieldOrPropertyValue<T>(string fieldName, object obj, bool includeAllBases = false, int? arrayIndex = null, BindingFlags bindings = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
		{
			if (arrayIndex != null)
			{
				FieldInfo arrayProperty = obj.GetType().GetField(fieldName, bindings);
				IList castedList = arrayProperty.GetValue(obj) as IList;
				return (T)castedList[arrayIndex.Value];
			}
		...
		public static bool SetFieldOrPropertyValue(string fieldName, object obj, object value, bool includeAllBases = false, int? arrayIndex = null, BindingFlags bindings = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
		{
			FieldInfo field = obj.GetType().GetField(fieldName, bindings);
			if (field != null)
			{
